...
bash.runCommand "${pname}-${version}" {
  ...
  # Configure
  export CC="gcc -D BOOTSTRAP_GLIBC=1 -I $(pwd)/nptl/sysdeps/pthread/bits -L ${glibc22}/lib"
  export CPP="gcc -E -D BOOTSTRAP_GLIBC=1 -I $(pwd)/nptl/sysdeps/pthread/bits"
  export LIBS="-lnss_files -lnss_dns -lresolv"
  export LD="gcc"
  ...
  mv Makefile .tmp
  cat .shell .tmp > Makefile
  mv ../Makefile .tmp
  cat .shell .tmp > ../Makefile
  mv ../Makeconfig .tmp
  cat .shell .tmp > ../Makeconfig
  mv ../elf/Makefile .tmp
  cat .shell .tmp > ../elf/Makefile
  rm .shell .tmp
