...
long write(int fd, const void* buf, long cnt) {
	return syscall(SYS_write, fd, (long) buf, cnt);
}

int execve(const char* fname,
		const char* const argv[], const char* const envp[]) {
	return syscall(SYS_execve, (long) fname, (long) argv, (long) envp);
}

void exit(int code) { syscall(SYS_exit, code); };
...
int mkdir(const char *pathname, unsigned mode) {
	return syscall(SYS_mkdir, (long) pathname, mode);
}

int open(const char *pathname, int flags, int mode) {
	return syscall(SYS_open, (long) pathname, flags, mode);
}
...
int getdents(unsigned int fd, struct linux_dirent *dirp,
		unsigned int count) {
	return syscall(SYS_getdents, fd, (long) dirp, count);
}
...
_Bool is_compileable(char* fname) {
	...
	if (i > 2)
		if (fname[i - 2] == '.')
			if (fname[i - 1] == 'c' || fname[i - 1] == 's' || fname[i - 1] == 'S')
				return 1;
	return 0;
}
...
#define PROTOMUSL_INTERNAL_INCLUDES \
		...
		"-I" PROTOSRC"/protomusl/arch/i386", \
...
void compile_protomusl(const char* cc) {
	...
	aa_extend_from_dir(&aa, 2, PROTOSRC"/protomusl/src/ldso/i386");
	...
	aa_extend_from_dir(&aa, 2, PROTOSRC"/protomusl/src/setjmp/i386");
	aa_extend_from_dir(&aa, 1, PROTOSRC"/protomusl/src/signal");
	aa_extend_from_dir(&aa, 2, PROTOSRC"/protomusl/src/signal/i386");
	...
	aa_extend_from_dir(&aa, 2, PROTOSRC"/protomusl/src/thread/i386");
...
void compile_standalone_busybox_applets(const char* cc) {
	...
	mass_compile(cc, (char*[]) {
		...
		"-DENABLE_LFS", // Required to be enabled on 32bit musl
