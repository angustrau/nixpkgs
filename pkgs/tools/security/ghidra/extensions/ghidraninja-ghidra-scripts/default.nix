...
buildGhidraScripts rec {
  ...
  postPatch = ''
    ...
    substituteInPlace binwalk.py --replace 'subprocess.call(["binwalk"' 'subprocess.call(["${binwalk}/bin/binwalk"'
    substituteInPlace swift_demangler.py --replace '"swift"' '"${swift}/bin/swift"'
    substituteInPlace yara.py --replace 'subprocess.check_output(["yara"' 'subprocess.check_output(["${yara}/bin/yara"'
    substituteInPlace YaraSearch.py --replace '"yara "' '"${yara}/bin/yara "' 
  '';
